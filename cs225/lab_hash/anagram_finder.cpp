...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
     ...
     */
     ...
     typename Dict<char, int>::iterator it2;
     bool anagram = false;
     for(it = wordHash.begin(); it != wordHash.end(); it++){    //iterate through each character in word
         anagram = false;
         for(it2 = testHash.begin(); it2 != testHash.end(); it2++){     //iterate through each character in test
             if(it->first == it2->first && it->second == it2->second){  //if there is an equal (char, int) pair
                 anagram = true;    //set to true
                 break; //break out of the loop
             }
         }
         if(!anagram){  //if a match wasnt found in the above for loop, it is not an anagram. break
             break;
         }
     }
     return anagram;
