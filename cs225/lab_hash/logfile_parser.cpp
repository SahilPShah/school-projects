...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
         ...
         * vector as well.
         */
         string data = ll.customer + ll.url;
         if(whenVisitedTable.keyExists(data)){
             if(difftime(whenVisitedTable[data], ll.date) < 0)
                whenVisitedTable.remove(data);

         }
         whenVisitedTable.insert(data, ll.date);
