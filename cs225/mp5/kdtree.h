...
class KDTree
{
  ...
  private:
     ...
     */
     int quickselect(vector<Point<Dim>>& pts, int start, int end, int pivot, int dim, int orig_start);
     /*
     ...
     */
     int partition(vector<Point<Dim>>& pts, int start, int end, int pivot, int dim);
     /*
     *recursively builds KDtree based on result from quickselect and the splitting dimension
     */
     KDTreeNode* buildTree(KDTreeNode* subroot, vector<Point<Dim>>& pts, int start, int end, int dim);
     /*
     *This function accepts a subroot and returns the root of a copied tree
     */
     KDTreeNode* copy(KDTreeNode* subroot);
     /*
     *This function accepts a subroot and deletes the tree rooted there.
     */
     void clear(KDTreeNode* subroot);
     /*
     *This function recursively finds the closed value to the query
     */
     void findClosest(KDTreeNode* subroot, const Point<Dim>& query, Point<Dim>& currentBest, int dim) const;
     /*
     *This function finds the Euclidean distance between two points
     */
     double distance(const Point<Dim>& target, const Point<Dim>& queryPoint) const;
