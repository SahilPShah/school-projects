...
HuffmanTree::removeSmallest(queue<TreeNode*>& singleQueue, queue<TreeNode*>& mergeQueue)
{
     ...
     */
     ...
     if(singleQueue.empty()){
       min=mergeQueue.front();
       mergeQueue.pop();
       return min;
     }
     if(mergeQueue.empty()){
       min=singleQueue.front();
       singleQueue.pop();
       return min;
     }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     //step one is to  create each leaf node and add them in sorted order to the singleQueue
     ...
     while(!(singleQueue.empty() && mergeQueue.empty())){
       left=removeSmallest(singleQueue,mergeQueue);
       right=removeSmallest(singleQueue,mergeQueue);
       //create internal node
       subroot = new TreeNode(left->freq.getFrequency()+right->freq.getFrequency());
       //set left and right
       subroot->left=left;
       subroot->right=right;
       //put onto merge queue
       mergeQueue.push(subroot);

       //if there is only one element remaining and it is on the merge queue
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
         ...
         */
         ...
         else if(bfile.getNextBit()){//if the bit is a one
    ...
    //add the last char to the stream
    if(!current->left && !current->right){
      //at a leaf node
      ss<<current->freq.getCharacter();
      current = root_;
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
     ...
     */
     if(!current->left && !current->right){//if at a leaf node
       bfile.writeBit(1);
