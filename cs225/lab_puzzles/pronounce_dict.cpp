...
{
    string word1_cap, word2_cap;
    word1_cap.resize(word1.size());
    word2_cap.resize(word2.size());
    std::transform(word1.begin(), word1.end(), word1_cap.begin(), ::toupper);
    std::transform(word2.begin(), word2.end(), word2_cap.begin(), ::toupper);
    if(!dict.count(word1_cap) || !dict.count(word2_cap))
      return false;
    else {
      vector<string> word1_vector = dict.find(word1_cap)->second;
      vector<string> word2_vector = dict.find(word2_cap)->second;
      return word1_vector == word2_vector;
    }
}
