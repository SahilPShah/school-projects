...
void backtracking( unsigned int index, int chosen[N], unsigned int permutation[N]  )
{
	int notUsed = 0,i;//emptyIndex is the index in the permutation that is empty. notUsed is the first number in the chosen array not used. i is a loop variable
	if ( index == N ){
		...
		// Your code goes here
		//check which number hasnt been used
		for(i=0;i<N;i++){
			if(chosen[i] ==0){
				notUsed = i+1;
			}
			permutation[index] = notUsed;
			index++;
			chosen[notUsed-1] = 1;
			backtracking(index,chosen[N],permutation[N]);
			chosen[notUsed-1] = 0;
		}
	}
