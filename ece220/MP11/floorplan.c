...
/*Sahil Shah
*sahils2
*This program is designed to exeute a floorplanning progam. This entails finding an arrangement of modules
*that yields the smaller area. The chip is represented using a tree data structure.
*/
...
int is_leaf_node(node_t* ptr) {
  if(ptr->left==NULL && ptr->right==NULL)//if both the right and left children are NULL
    return TRUE;
  return FALSE
}
...
//recursively call the function to determine if subtree b is in subtree A
int is_in_subtree(node_t* a, node_t* b){
    if(a==NULL)//base case
        return FALSE;
    if(a==b)//pointer a and b have the same address (i.e, the same node)
        return TRUE;
    return (is_in_subtree(a->left, b) || is_in_subtree(a->right,b));//recursively call function
                                                                        //to check if the right or left of root A matches B
}
...
void rotate(node_t* ptr) {
  int temp = (ptr->module)->h;//store the height in a temp variable
  (ptr->module)->h=(ptr->module)->w;//set the new height to the width
  (ptr->module)->w=temp;//set the new width to the old height (the temp variable)
}
...
void recut(node_t* ptr) {
  ...
  assert(ptr->module == NULL && ptr->cutline != UNDEFINED_CUTLINE);//assume the cutline is not undefined
  ptr->cutline==V?ptr->cutline=H:ptr->cutline=V;
  return;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  module_t* temp = a->module;//store module of a in a temp variable
  a->module=b->module;//put b into a
  b->module=temp;//put old a into b;
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  node_t** childA=(a->parent)->left==a?&(a->parent)->left:&(a->parent)->right;//pointer to the specfic child in the tree
  node_t** childB=(b->parent)->left==b?&(b->parent)->left:&(b->parent)->right;//pointer to the specific child in the tree
  *childA = b;
  *childB = a;//swap the children
  node_t* temp = a->parent;
  a->parent = b->parent;
  b->parent = temp;//swap the parents
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  //get module and cutline data
  expression[*nth].module=ptr->module;
  expression[*nth].cutline=ptr->cutline;
  //increment the current index of the expression array
  (*nth)++;
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  node_t* ptr = (node_t*)malloc(sizeof(node_t));//allocate space for a node pointer
  if(n==num_modules-1){//if we are at the end of the module array. End of initialization
      //initialize node
      ptr->parent=par;
  ...
  //if not at the end
    ptr->parent = par;
    ptr->module = NULL;//this is a cutline so the module == NULL
    ...
    ptr->left=init_slicing_tree(ptr,n+1);//call function for next left child and increment n to access nxt module in the array
    return ptr;
}
