...
void entry(unsigned long magic, unsigned long addr) {
    ...
    uint32_t filesys_addr;      // memory address where file system starts
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        filesys_addr = mod->mod_start;      // memory address where file system starts
    ...
    /*initialize pic, keyboard, and rtc*/
    i8259_init();     // initialize PIC and devices
    pit_init();
    kbd_init();
    rtc_init();

    terms_init = 0;
    terminal_open();  // open terminal instance
    exceptions_flag = 0;
