...
/* interrupt descriptor values*/
#define TRAP_desc	1
#define INT_desc	0
#define PRESENT_desc 1
#define KERNEL_dpl 0
#define USER_dpl 3


/* interrupt locations */
#define KEYBOARD_INT 	0x0060 // <<<<<<<<<<<<<< PIC VALUE UPDATE, after keyboard.h is ready
...
uint32_t sys_call_table[NUM_SYSC];
...
/* Task descriptor entry, p156 in big manual */
typedef union task_desc_t{
	uint32_t val[2];
    struct {
        uint16_t padding;
        uint16_t seg_selector;
        uint8_t  padding2;
        uint32_t reserved4 : 1; // = 1
...
/* An interrupt/trap descriptor entry (goes into the IDT) */
typedef union idt_desc_t {
    uint32_t val[2];
    struct {
        uint16_t offset_15_00;
        uint16_t seg_selector;
        uint8_t  reserved4;		// all equal zero
        uint32_t reserved3 : 1; // = 0 for INT, = 1 for Trap
        uint32_t reserved2 : 1;	// = 1
        uint32_t reserved1 : 1; // = 1
        uint32_t size      : 1; // = d (1 = 32 bits, 0 = 16 bits)
        uint32_t reserved0 : 1; // = 0
        uint32_t dpl       : 2;
        uint32_t present   : 1;
